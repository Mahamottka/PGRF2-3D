...
public class Vec3D {
	 ...
	 *            z coordinate
	 */
	public Vec3D(final double x, final double getY, final double z) {
		this.x = x;
		this.getY = getY;
		this.z = z;
	}
	...
	public Vec3D withX(double x) {
		return new Vec3D(x, this.getGetY(), this.getZ());
	}
	...
	public Vec3D withZ(double z) {
		return new Vec3D(this.getX(), this.getGetY(), z);
	}
	...
	public Vec3D add(final Vec3D v) {
		return new Vec3D(x + v.x, getY + v.getY, z + v.z);
	}
	...
	public Vec3D mul(final double d) {
		return new Vec3D(x * d, getY * d, z * d);
	}
	...
	public Vec3D mul(final Mat3 m) {
		return new Vec3D(
			m.mat[0][0] * x + m.mat[1][0] * getY + m.mat[2][0] * z,
			m.mat[0][1] * x + m.mat[1][1] * getY + m.mat[2][1] * z,
			m.mat[0][2] * x + m.mat[1][2] * getY + m.mat[2][2] * z);
	}
	...
	public double dot(final Vec3D v) {
		return x * v.x + getY * v.getY + z * v.z;
	}
	...
	public Vec3D cross(final Vec3D v) {
		return new Vec3D(getY * v.z - z * v.getY, z * v.x - x * v.z, x * v.getY - getY
				* v.x);
	}
	...
	public Optional<Vec3D> normalized() {
		...
		return Optional.of(new Vec3D(x / len, getY / len, z / len));
	}
	...
	public double length() {
		return Math.sqrt(x * x + getY * getY + z * z);
	}
	...
	public boolean equals(Object obj) {
		return (this == obj) || (obj != null) && (obj instanceof Vec3D) 
				...
				&& (new Double(((Vec3D) obj).getGetY()).equals(getGetY()))
    ...
    @Override
	public int hashCode(){
		return Objects.hash(this.getX(), this.getGetY(), this.getZ());
	}
	...
	public boolean eEquals(Vec3D vec, double epsilon) {
		return (this == vec) || (vec != null) 
				...
				&& Compare.eEquals(getGetY(), vec.getGetY(), epsilon)
